...
public class DataManager : MonoBehaviour
{
    ...
    private void MakeSheetDatset(DataSet dataset)
    {
        ...
        pass.ClientId = "200447528248-a5cu6bn2v10b1fho7ui0o26n2dt2nulb.apps.googleusercontent.com"; //QAuth 2.0 클라이언트 ID
        pass.ClientSecret = "AdrorKxSHgq_qpWpRcya-3vr"; //QAuth 2.0 클라이언트 PW

        var scopes = new string[] { SheetsService.Scope.SpreadsheetsReadonly };
        var credential = GoogleWebAuthorizationBroker.AuthorizeAsync(pass, scopes, "SpinningColor", CancellationToken.None).Result;

        var service = new SheetsService(new BaseClientService.Initializer()
        {
            HttpClientInitializer = credential,
            ApplicationName = "SpinningColor"
        });
        
        var request = service.Spreadsheets.Get("1awTXAGWEpK8l8EUep7QcezDcvIaPD5zw_wUAWozMKz8").Execute(); //스프레드시트 URL에서 따옴
    ...
    private DataTable SendRequest(SheetsService service, string sheetName) //스프레드시트 파일에 존재하는 시트들의 목록을 받아와서 시트별로 데이터를 받아옴
    {
        ...
        try
        {
            ...
            var request = service.Spreadsheets.Values.Get("1awTXAGWEpK8l8EUep7QcezDcvIaPD5zw_wUAWozMKz8", sheetName + "!A1:M");//스프레드시트 URL에서 따옴
